RSpec.describe Manual::RepublishService do
  ...
  before do
    ...
    allow(Publishing::PublishAdapter).to receive(:publish_manual_and_sections)
  ...
  context "(for a published manual)" do
    ...
    it "calls the new publishing api publisher" do
      described_class.call(user:, manual_id:)
      expect(Publishing::PublishAdapter).to have_received(:publish_manual_and_sections).with(published_manual_version, republish: true)
    end
    ...
    it "does not publish attachment assets" do
      described_class.call(user:, manual_id:)
  ...
  context "(for a draft manual)" do
    ...
    it "tells the published listeners nothing" do
      described_class.call(user:, manual_id:)
      expect(Publishing::PublishAdapter).not_to have_received(:publish_manual_and_sections)
