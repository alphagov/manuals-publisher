class PublishManualWorker
  ...
  def perform(task_id)
    ...
    task.finish!
  rescue PanopticonRegisterer::ServerError,
         ManualPublishingAPIExporter::ServerError => error
    log_error(error)
    requeue_task(task_id, error)
  rescue PublishManualService::VersionMismatchError,
         PanopticonRegisterer::ClientError,
         ManualPublishingAPIExporter::ClientError,
         StandardError => error
    log_error(error)
